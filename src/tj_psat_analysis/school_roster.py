...
HISTORY_EVENTS: tuple[dict[str, object], ...] = (
    {
        ...
        "source_title": "Washington-Liberty High School History and Timeline",
        "source_url": "https://wl.apsva.us/about-us/w-l-history-and-timeline/",
        "source_date": "2019-07-01",
        "source_note": "APS states that the Washington-Liberty name became effective July 1, 2019.",
    },
    {
...
def _alias_candidates(record: SchoolRecord) -> Iterable[tuple[str, str, str, str]]:
    ...
    for alias, alias_type, note in EXTRA_ALIASES_BY_SCHOOL_ID.get(record.school_id, ()):
        yield alias, alias_type, "manual_roster_alias", note

    for alias, alias_type, note in SOURCE_ALIASES_BY_SCHOOL_ID.get(record.school_id, ()):
        yield alias, alias_type, "local_media_source_alias", note
